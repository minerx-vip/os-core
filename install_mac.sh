...
download_file() {
    ...
    if wget -t 5 -T 20 -c "${url}" -P /tmp/; then
        ...
        tar xzf ${archive} -C ${install_path} || { echoYellow "Install failed!"; exit 1; }
        return 0
    else
...
##################################################################
RIG_CONFIG="$HOME/os/config/rig.conf"
...
if [[ ! -z ${farmid} ]]; then
    [[ ! -f $HOME/os/config/rig.conf ]] && touch $HOME/os/config/rig.conf
    
    # 兼容 macOS 的 sed 命令
    if [[ "$OS_TYPE" == "Darwin" ]]; then
        sed -i '' '/^farm_hash/d' $HOME/os/config/rig.conf
        sed -i '' '/^worker_name/d' $HOME/os/config/rig.conf
        sed -i '' '/^server_url/d' $HOME/os/config/rig.conf
    fi

    echo "farm_hash=${farmid}" >> $HOME/os/config/rig.conf
    echo "worker_name=`hostname`" >> $HOME/os/config/rig.conf
    echo 'server_url="http://47.97.210.214:8888"' >> $HOME/os/config/rig.conf
    echo 'server_url_domain="https://api.minerx.vip"' >> $HOME/os/config/rig.conf
fi
...
# ##################################################################
$HOME/os/bin/say-hello-mac register
